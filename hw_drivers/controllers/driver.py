...
class StatusController(http.Controller):
    ...
    def connect_box(self, token):
        """
        This route is called when we want that a IoT Box will be connected to a ALWAFI DB
        token is a base 64 encoded string and have 2 argument separate by |
        1 - url of odoo DB
        2 - token. This token will be compared to the token of ALWAFI. He have 1 hour lifetime
        """
...
class Manager(Thread):
    ...
    def send_alldevices(self):
        """
        This method send IoT Box and devices informations to ALWAFI database
        """
        server = get_odoo_server_url()
        if server:
            iot_box = {
            ...
            _logger.warning('ALWAFI server not set')
