...
@tagged("odoobot")
class TestALWAFIbot(BaseFunctionalTest, MockEmails, TestRecipients):

    def setUp(self):
        super(TestALWAFIbot, self).setUp()
        ...
        self.odoobot_ping_body = '<a href="http://odoo.com/web#model=res.partner&amp;id=%s" class="o_mail_redirect" data-oe-id="%s" data-oe-model="res.partner" target="_blank">@ALWAFIBot</a>' % (self.odoobot.id, self.odoobot.id)
    ...
    def test_odoobot_ping(self):
        ...
        with patch('random.choice', lambda x: x[0]):
            self.assertNextMessage(
                ...
                sender=self.odoobot,
                answer=["Yep, ALWAFIBot is in the place!"]
            )
        # ALWAFIbot should not be a follower but user_employee and user_admin should
