...
var MailManager =  AbstractService.extend({
    ...
    /**
     * Get the ALWAFIBot ID, which is the default authorID for transient messages
     *
     * @returns {string}
     */
    getALWAFIbotID: function () {
            ...
            if (thread) {
                if (
                    ...
                    if (thread.isTwoUserThread() && options.showNotification) {
                        ...
                        if (!self.call('bus_service', 'isALWAFIFocused') || !query.isVisible) {
    ...
    _notifyIncomingMessage: function (message) {
        if (this.call('bus_service', 'isALWAFIFocused')) {
            // no need to notify
            return;
        }
