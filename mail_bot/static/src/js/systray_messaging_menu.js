...
return MessagingMenu.include({
     ...
     * messaging menu (e.g. 'ALWAFIBot has a request')
     *
     * @override
     * @private
     ...
     * ALWAFIBot, and the ALWAFIBot influences the counter by 1 when it requests
    ...
    _handleResponseNotificationPermission: function (value) {
        ...
        if (value !== 'granted') {
            this.call('bus_service', 'sendNotification', _t('Permission denied'),
                _t('ALWAFI will not have the permission to send native notifications on this device.'));
        } else {
            this.call('bus_service', 'sendNotification', _t('Yay, push notifications are enabled!'),
                _t('ALWAFI has now the permission to send you native notifications on this device.'));
        }
        this._updateCounter();
    },
    /**
    ...
    /**
     * Override so that it handles preview related to ALWAFIBot
     *
     * @override
     * @private
     * @param {MouseEvent} ev
