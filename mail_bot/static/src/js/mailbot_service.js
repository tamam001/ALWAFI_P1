...
var MailBotService =  AbstractService.extend({
    ...
    /**
     * Get the previews related to the ALWAFIBot (conversation not included).
     * For instance, when there is no conversation with ALWAFIBot and ALWAFIBot has
    ...
    getPreviews: function (filter) {
        ...
        if (this.hasRequest() && (filter === 'mailbox_inbox' || !filter)) {
            previews.push({
                title: _t("ALWAFIBot has a request"),
    ...
    /**
     * Tell whether ALWAFIBot has a request or not.
     *
     * @returns {boolean}
     */
    hasRequest: function () {
