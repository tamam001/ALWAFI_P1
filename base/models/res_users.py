...
class Users(models.Model):
    ...
    def unlink(self):
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by ALWAFI (updates, module installation, ...)'))
        db = self._cr.dbname
        for id in self.ids:
            self.__uid_cache[db].pop(id, None)
        self._invalidate_session_cache()
        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warn(
                    ...
                    "and *might* be a proxy. If your ALWAFI is behind a proxy, "
                    "it may be mis-configured. Check that you are running "
                    "ALWAFI in Proxy Mode and that the proxy is properly configured, see "
