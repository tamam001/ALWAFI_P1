...
class IoTboxHomepage(web.Home):
    ...
    def get_hw_screen_message(self):
        ...
        <p>
            ...
            You can find the latest images on the <a href="http://nightly.odoo.com/master/posbox/">ALWAFI Nightly builds</a> website.
            Make sure to download at least the version 16.<br/>
            ALWAFI version 11, or above, is required to use the customer display feature.
        </p>
        """
    ...
    def list_drivers(self):
        ...
        return driver_list_template.render({
            'title': "ALWAFI's IoT Box - Drivers list",
    ...
    def connect_to_wifi(self, essid, password, persistent=False):
        ...
        if server:
            res_payload['server'] = {
                'url': server,
                'message': 'Redirect to ALWAFI Server'
            }
    ...
    def server(self):
        return server_config_template.render({
            'title': 'IoT -> ALWAFI server configuration',
            'breadcrumb': 'Configure ALWAFI Server',
            'hostname': subprocess.check_output('hostname').decode('utf-8').strip('\n'),
